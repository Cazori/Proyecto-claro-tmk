import os
import json
import pandas as pd
from datetime import datetime
from supabase import create_client, Client
from dotenv import load_dotenv
from tenacity import retry, stop_after_attempt, wait_exponential, retry_if_exception_type

load_dotenv()

url = os.environ.get("SUPABASE_URL")
key = os.environ.get("SUPABASE_KEY")

supabase: Client = create_client(url, key) if url and key else None

# Retry policy: exponential backoff 1s, 2s, 4s, max 3 attempts
SUPABASE_RETRY = retry(
    wait=wait_exponential(multiplier=1, min=1, max=4),
    stop=stop_after_attempt(3),
    retry=retry_if_exception_type(Exception),
    reraise=True
)

def _sb():
    """Lazy getter — re-crea cliente si env vars cambiaron (útil en serverless)."""
    global supabase
    if supabase is None and url and key:
        supabase = create_client(url, key)
    return supabase

# --- DATABASE LOGIC ---

@SUPABASE_RETRY
async def save_inventory_to_db(df: pd.DataFrame):
    sb = _sb()
    if sb is None: return
    
    # 1. Prepare safe records (only send columns that exist in the remote schema)
    # Based on discovery, remote only has: Material, Subproducto, categoria, marca, modelo_limpio, especificaciones
    verified_cols = ['Material', 'Subproducto', 'CantDisponible', 'Precio Contado', 'categoria', 'marca', 'modelo_limpio', 'especificaciones', 'tip_venta']
    
    safe_df = df.copy()
    if "Precio Cuotas" in safe_df.columns:
        safe_df["Precio Contado"] = safe_df["Precio Cuotas"]
        
    available_cols = [c for c in verified_cols if c in safe_df.columns]
    safe_df = safe_df[available_cols]
    
    records = safe_df.to_dict('records')
    
    # 2. Clear previous inventory
    try:
        sb.table('inventory').delete().neq('Material', '0').execute()
        
        # 3. Batch insert
        chunk_size = 500
        for i in range(0, len(records), chunk_size):
            chunk = records[i:i + chunk_size]
            sb.table('inventory').insert(chunk).execute()
            
        # 4. Update metadata
        await update_metadata_db()
        print(f"✓ {len(records)} productos persistidos en Supabase DB.")
    except Exception as e:
        print(f"✗ Error guardando en Supabase: {e}")

@SUPABASE_RETRY
async def get_inventory_from_db(columns: str = "*"):
    sb = _sb()
    if sb is None: return None
    try:
        # Optimization: Fetching only what we need reduces memory usage on Render
        response = sb.table('inventory').select(columns).execute()
        if response.data:
            df = pd.DataFrame(response.data)
            if "Precio Contado" in df.columns:
                df["Precio Cuotas"] = df["Precio Contado"]
            return df
    except Exception as e:
        print(f"✗ Error leyendo de Supabase: {e}")
    return None

@SUPABASE_RETRY
async def update_metadata_db():
    sb = _sb()
    if sb is None: return
    now = datetime.now().isoformat()
    try:
        sb.table('metadata').upsert({"id": 1, "last_update": now, "status": "ready"}).execute()
    except Exception as e:
        print(f"✗ Error actualizando metadata: {e}")

@SUPABASE_RETRY
async def get_metadata_db():
    sb = _sb()
    if sb is None: return None
    try:
        response = sb.table('metadata').select("*").eq("id", 1).execute()
        if response.data:
            return response.data[0]
    except Exception as e:
        print(f"✗ Error obteniendo metadata: {e}")
    return None

@SUPABASE_RETRY
async def save_quotas_to_db(mapping: dict):
    sb = _sb()
    if sb is None: return
    try:
        # We store the quota mapping as a single JSON object in a dedicated table 'quotas'
        # with id=1 for simplicity (or we could store it as records per SKU)
        # Let's use a simple key-value approach for the whole JSON for speed
        payload = {"id": 1, "data": mapping, "updated_at": datetime.now().isoformat()}
        sb.table('quotas').upsert(payload).execute()
        print(f"✓ Mapeo de cuotas ({len(mapping)} equipos) guardado en Supabase.")
    except Exception as e:
        print(f"✗ Error guardando cuotas en Supabase: {e}")

@SUPABASE_RETRY
async def get_quotas_from_db():
    sb = _sb()
    if sb is None: return None
    try:
        response = sb.table('quotas').select("data").eq("id", 1).execute()
        if response.data:
            return response.data[0]["data"]
    except Exception as e:
        print(f"✗ Error leyendo cuotas de Supabase: {e}")
    return None

# --- CUSTOM MAPPINGS PERSISTENCE (JSON) ---

@SUPABASE_RETRY
async def save_specs_mapping_to_db(mapping: dict):
    sb = _sb()
    if sb is None: return
    try:
        payload = {"id": 1, "data": mapping, "updated_at": datetime.now().isoformat()}
        sb.table('specs_mapping').upsert(payload).execute()
        print(f"✓ Mapeo de imágenes sincronizado con Supabase.")
    except Exception as e:
        print(f"✗ Error sincronizando mapeo de imágenes: {e}")

@SUPABASE_RETRY
async def get_specs_mapping_from_db():
    sb = _sb()
    if sb is None: return None
    try:
        response = sb.table('specs_mapping').select("data").eq("id", 1).execute()
        if response.data:
            return response.data[0]["data"]
    except Exception as e:
        print(f"✗ Error leyendo mapeo de imágenes de Supabase: {e}")
    return None

@SUPABASE_RETRY
async def save_knowledge_to_db(knowledge: list):
    sb = _sb()
    if sb is None: return
    try:
        payload = {"id": 1, "data": knowledge, "updated_at": datetime.now().isoformat()}
        sb.table('expert_knowledge').upsert(payload).execute()
        print(f"✓ Conocimiento experto sincronizado con Supabase.")
    except Exception as e:
        print(f"✗ Error sincronizando conocimiento experto: {e}")

@SUPABASE_RETRY
async def get_knowledge_from_db():
    sb = _sb()
    if sb is None: return None
    try:
        response = sb.table('expert_knowledge').select("data").eq("id", 1).execute()
        if response.data:
            return response.data[0]["data"]
    except Exception as e:
        print(f"✗ Error leyendo conocimiento experto de Supabase: {e}")
    return None

# --- STORAGE LOGIC (SPECS) ---

@SUPABASE_RETRY
async def upload_spec_to_supabase(file_path: str, filename: str):
    sb = _sb()
    if sb is None: return
    try:
        with open(file_path, 'rb') as f:
            sb.storage.from_('specs').upload(
                path=filename,
                file=f,
                file_options={"cache-control": "3600", "upsert": "true"}
            )
        print(f"✓ Ficha {filename} subida a Supabase Storage.")
    except Exception as e:
        print(f"✗ Error subiendo ficha a Storage: {e}")

def get_spec_url_supabase(filename: str):
    sb = _sb()
    if sb is None: return None
    return sb.storage.from_('specs').get_public_url(filename)

@SUPABASE_RETRY
async def list_specs_supabase():
    sb = _sb()
    if sb is None: return []
    try:
        response = sb.storage.from_('specs').list()
        return [f['name'] for f in response]
    except Exception as e:
        print(f"✗ Error listando fichas: {e}")
        return []

# --- STORAGE LOGIC (INVENTORY PDF) ---

@SUPABASE_RETRY
async def upload_inventory_pdf_to_supabase(file_path: str, filename: str):
    sb = _sb()
    if sb is None: return
    try:
        with open(file_path, 'rb') as f:
            sb.storage.from_('inventories').upload(
                path=filename,
                file=f,
                file_options={"cache-control": "3600", "upsert": "true"}
            )
        print(f"✓ PDF {filename} subido a Supabase Storage.")
    except Exception as e:
        print(f"✗ Error subiendo PDF a Storage: {e}")

@SUPABASE_RETRY
async def download_latest_inventory_pdf_from_supabase(local_dir: str):
    sb = _sb()
    if sb is None: return None
    try:
        files = sb.storage.from_('inventories').list()
        if not files: return None
        
        files.sort(key=lambda x: x.get('created_at', ''), reverse=True)
        latest_filename = files[0]['name']
        
        local_path = os.path.join(local_dir, latest_filename)
        with open(local_path, 'wb') as f:
            res = sb.storage.from_('inventories').download(latest_filename)
            f.write(res)
        
        print(f"✓ PDF {latest_filename} descargado de Supabase Storage.")
        return local_path
    except Exception as e:
        print(f"✗ Error descargando PDF de Supabase: {e}")
        return None
